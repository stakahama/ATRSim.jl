...
The following figure depicting the magnitude of particle modification factor is slightly different from Figure S2 (bottom panel) of Arangio et al. The version below is calculated assuming the penetration depth is not be influenced by the sample medium n₂ (bulk), but instead nₐᵢᵣ (thin flim) (Mirabella, 1985).
```{julia}
#| output: true
pl = plot(layout = (2, 1))
