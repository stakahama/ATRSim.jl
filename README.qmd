...
- `deff`: effective thickness for thick sample (`n₂₁ * E₀² / cos(θ) * dp / 2`)
- `ξ`, `xi` (non-unicode alias): `deff / d` for thin sample (`n₂₁ * E₀² / cos(θ)`)
- `χ`, `chi` (non-unicode alias): particle factor
- `prefactor`: `N / aIRE * ξ * χ`

Arguments to many of these functions take the form of vectors `ν`, `λ`, and refractive indices. They are assumed to be aligned a priori.
...
The optional last argument defines the refractive index of the third medium above the sample (air) when the sample in contact with the IRE is prepared as a thin film. The data type of variable returned determines the behavior of some functions (e.g., penetration depth, electric field intensity at the interface).
```{julia}
nₐᵢᵣ = 1.0
